...
export function prepareSphereGenerator(PERLIN_EPSILON: number, PERLIN_STRENGTH: number, SPHERE_DIVISIONS: number)
{
  function getVertexAmount() {
  ...
  function generateSphere(root: TgpuRoot, perlinOffset: number, isSphere: number) {
  ...
  return { getVertexAmount, generateSphere };
}